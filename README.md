...
You can use these commands, e.g. `QueuedBackgroundTasks`  gets renamed to `MyApp`:
...
```shell
LC_ALL=C find . -type f -name '*.*' -not \( -path './node_modules/*' -o -path './src/QueuedBackgroundTasks/node_modules/*' -o -path './assets' \) -exec sed -i '' 's|QueuedBackgroundTasks|MyApp|g' {} \;
```
...
```bash
find . -depth -name "*QueuedBackgroundTasks*" | \
while IFS= read -r ent; do mv $ent ${ent%QueuedBackgroundTasks*}MyApp${ent##*QueuedBackgroundTasks}; done
